...
Phase 3 已接入：router → planner → collector → analyzer → thesis → synthesizer → END
（researcher / evaluator 在后续 Phase 接入）。
...
from nodes import AnalyzerNode, CollectorNode, ThesisNode, planner, router, synthesizer
...
def build_graph(
    ...
    graph.add_edge("thesis", "synthesizer")
